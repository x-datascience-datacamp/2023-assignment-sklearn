...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        for i, x in enumerate(X):
            ...
            index = np.argsort(distances, axis=1)[:, :self.n_neighbors]
            values, counts = np.unique(self.y_[index], return_counts=True)
...
class MonthlySplit(BaseCrossValidator):
    ...
    def split(self, X, y, groups=None):
        ...
        indexes = [group.index for _, group in X_grouped]
        for i in range(n_splits):
            index_train = list(indexes[i])
            index_test = list(indexes[i+1])
            yield (
