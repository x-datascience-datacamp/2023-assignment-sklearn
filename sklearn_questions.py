...
class MonthlySplit(BaseCrossValidator):
    ...
    def split(self, X, y, groups=None):
        ...
        X_grouped = X_sorted.groupby(pd.Grouper(key=self.time_col,
                                                freq="M"))
        indices = [group.index for _, group in X_grouped]

        for i in range(n_splits):
            idx_train = indices[i].tolist()
            idx_test = indices[i+1].tolist()
