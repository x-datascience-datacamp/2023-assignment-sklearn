...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        def convert_pred(X):
            elem, counts = np.unique(self.y_[X], return_counts=True)
            pred = elem[np.argmax(counts)]
            return pred
        X = check_array(X)
        closest = np.argsort(euclidean_distances(X, self.X_),
                             axis=1)[:, :self.n_neighbors]
        prediction = np.apply_along_axis(convert_pred, 1, closest)
        return prediction
...
class MonthlySplit(BaseCrossValidator):
    ...
    def split(self, X, y, groups=None):
        ...
        for i in range(n_splits):
            ...
            train = indexes[mask_trainning]
            test = indexes[mask_testing]
            yield (
                train, test
            )
