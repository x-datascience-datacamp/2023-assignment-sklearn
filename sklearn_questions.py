...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        for row in nearest_labels:
            c = Counter(row).most_common(1)[0][0]
            y_pred.append(c)
...
class MonthlySplit(BaseCrossValidator):
    ...
    def get_n_splits(self, X, y=None, groups=None):
        ...
        # Calculate the number of months available for splitting
        months = pd.to_datetime(X[self.time_col]).dt.to_period("M").nunique()
        return months - 1  # We return months - 1 for splitting
